...
static inline SSE_FLOATS sse_pair_product(SSE_FLOATS, SSE_FLOATS, SSE_FLOATS, SSE_FLOATS w0, SSE_FLOATS w1){
    ...
    for(int64_t i=0;i<N0;i++) {
        ...
        */
        ...
        for(;j<=(N1 - SSE_NVEC);j+=SSE_NVEC){
            ...
            if(need_wavg){
                union_mweight.m_weights = sse_weight_func(m_xdiff, m_ydiff, m_zdiff, m_wi, m_wj);
            }
