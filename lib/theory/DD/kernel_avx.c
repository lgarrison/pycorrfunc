...
typedef AVX_FLOATS (*avx_weight_func_t)(
   AVX_FLOATS dx, AVX_FLOATS dy, AVX_FLOATS dz, AVX_FLOATS w0, AVX_FLOATS w1);
avx_weight_func_t get_avx_weight_func_by_method(const weight_method_t method) {
    switch (method) {
        case PAIR_PRODUCT: return &avx_pair_product;
        default:
        case NONE: return NULL;
    }
}

int countpairs_avx(uint64_t *restrict src_npairs,
    ...
    for (int64_t i = 0; i < N0; i++) {
        ...
        */
        ...
        for (; j <= (N1 - AVX_NVEC); j += AVX_NVEC) {
            ...
            localx1 += AVX_NVEC;  // this might actually exceed the allocated range but
                                  // we will never dereference that
            localy1 += AVX_NVEC;
            localz1 += AVX_NVEC;
            ...
            const AVX_FLOATS m_zdiff =
               AVX_SUBTRACT_FLOATS(m_z1, m_zpos);  // z2[j:j+NVEC-1] - z1
            ...
            AVX_FLOATS r2 =
               AVX_ADD_FLOATS(m_sqr_zdiff, AVX_ADD_FLOATS(m_sqr_xdiff, m_sqr_ydiff));
            ...
            // Do all the distance cuts using masks here in new scope
            {
                ...
                AVX_FLOATS m_mask_geq_pimax =
                   AVX_COMPARE_FLOATS(m_zdiff, m_max_dz, _CMP_GE_OS);
                if (AVX_TEST_COMPARISON(m_mask_geq_pimax) > 0) {
                    j = N1;  // but do not break yet, there might be valid pairs in this
                             // chunk
                }

                const AVX_FLOATS m_rmax_mask =
                   AVX_COMPARE_FLOATS(r2, m_sqr_rmax, _CMP_LT_OS);
                const AVX_FLOATS m_rmin_mask =
                   AVX_COMPARE_FLOATS(r2, m_sqr_rmin, _CMP_GE_OS);
                ...
                if (same_cell == 0 && need_ravg == 0 && need_wavg == 0) {
                    const AVX_FLOATS m_last_bin = AVX_BITWISE_AND(
                       m_mask_left,
                       AVX_COMPARE_FLOATS(r2, m_edge_sqr[nbinedge - 2], _CMP_GE_OS));
                    if (AVX_TEST_COMPARISON(m_last_bin)
                        == num_left) { /* all the valid pairs are in the last bin */
                        src_npairs[nbinedge - 2] +=
                           num_left; /* add the total number of pairs to the last bin
                                        and continue j-loop*/
                        continue;
                    }
                }

                // There is some r2 that satisfies sqr_rmin <= r2 < sqr_rmax && 0.0 <=
                // dz^2 < pimax^2.
                r2 = AVX_BLEND_FLOATS_WITH_MASK(m_sqr_rmax, r2, m_mask_left);
            }

            AVX_FLOATS m_rbin = AVX_SETZERO_FLOAT();
            if (need_ravg) { union_mDperp.m_Dperp = AVX_SQRT_FLOAT(r2); }
            if (need_wavg) {
                union_mweight.m_weights =
                   avx_weight_func(m_xdiff, m_ydiff, m_zdiff, m_wi, m_wj);
            }
            ...
            for (int kbin = nbinedge - 2; kbin >= 0; kbin--) {
                const AVX_FLOATS m1 =
                   AVX_COMPARE_FLOATS(r2, m_edge_sqr[kbin], _CMP_GE_OS);
                const AVX_FLOATS m_bin_mask = AVX_BITWISE_AND(m1, m_mask_left);
                const int test2             = AVX_TEST_COMPARISON(m_bin_mask);
                src_npairs[kbin] += AVX_BIT_COUNT_INT(test2);
                if (need_ravg || need_wavg) {
                    m_rbin =
                       AVX_BLEND_FLOATS_WITH_MASK(m_rbin, m_kbin[kbin], m_bin_mask);
                }
        ...
        // remainder loop
        // pair.weights0[w].d was set as an AVX float, but is still valid as a DOUBLE
        // here
        for (; j < N1; j++) {
