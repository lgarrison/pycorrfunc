...
static inline AVX512_FLOATS avx512_pair_product(AVX512_FLOATS, AVX512_FLOATS, AVX512_FLOATS, AVX512_FLOATS w0, AVX512_FLOATS w1){
    ...
    const weight_method_t weight_method)
{
    ...
    avx512_weight_func_t avx512_weight_func = get_avx512_weight_func_by_method(weight_method);
    ...
    for(int64_t i=0;i<N0;i++) {
        ...
        */
        ...
        for(int64_t j=n_off;j<N1;j+=AVX512_NVEC) {
            ...
            if(need_wavg){
                union_mweight.m_weights = avx512_weight_func(m_xdiff, m_ydiff, m_zdiff, m_wi, m_wj);
            }
            ...
            for(int kbin=nbinedge-2;kbin>=0;kbin--) {
                const AVX512_MASK m_bin_mask = AVX512_MASK_COMPARE_FLOATS(m_mask_left, r2,m_edge_sqr[kbin],_CMP_GE_OS);
                src_npairs[kbin] += bits_set_in_avx512_mask[m_bin_mask];
                if(need_ravg || need_wavg) {
                    m_rbin = AVX512_BLEND_INTS_WITH_MASK(m_bin_mask, m_rbin, AVX512_SET_INT(kbin + 1));
                }
