...
kernel_func_ptr countpairs_driver(const config_options *options) {
    ...
    // If options.isa != FASTEST, use the specified ISA and raise an error if it is not
    // available, without falling back.
    ...
    switch (options->instruction_set) {
        ...
        case FALLBACK:
            ...
            break;
        default: sprintf(ERRMSG, "Unknown ISA\n"); return NULL;
    }
...
int countpairs(const int64_t ND1,
    ...
    if (countpairs_function == NULL) { return EXIT_FAILURE; }
    ...
    if (options->autocorr == 0) {
        if (options->verbose) {
            fprintf(
               stderr,
               "ND1 = %12" PRId64
               " [xmin,ymin,zmin] = [%lf,%lf,%lf], [xmax,ymax,zmax] = [%lf,%lf,%lf]\n",
               ND1,
               xmin,
               ymin,
               zmin,
    ...
    if (options->verbose) {
        if (periodic_x) {
            fprintf(
               stderr,
               "Running with points in [xmin,xmax] = %lf,%lf with periodic wrapping = %lf\n",
               xmin,
               xmax,
               xwrap);
        } else {
            fprintf(stderr,
                    "Running with points in [xmin,xmax] = %lf,%lf (non-periodic)\n",
                    xmin,
                    xmax);
        }
        if (periodic_y) {
            fprintf(
               stderr,
               "Running with points in [ymin,ymax] = %lf,%lf with periodic wrapping = %lf\n",
               ymin,
               ymax,
               ywrap);
        } else {
            fprintf(stderr,
                    "Running with points in [ymin,ymax] = %lf,%lf (non-periodic)\n",
                    ymin,
                    ymax);
        }
        if (periodic_z) {
            fprintf(
               stderr,
               "Running with points in [zmin,zmax] = %lf,%lf with periodic wrapping = %lf\n",
               zmin,
               zmax,
               zwrap);
        } else {
            fprintf(stderr,
                    "Running with points in [zmin,zmax] = %lf,%lf (non-periodic)\n",
                    zmin,
                    zmax);
        }
    }
    ...
    if (get_grid_refine_scheme(options) == GRIDDING_DFL) {
        ...
        if (rmax < 0.05 * xwrap) { options->grid_refine_factors[0] = 1; }
        if (rmax < 0.05 * ywrap) { options->grid_refine_factors[1] = 1; }
        if (rmax < 0.05 * zwrap) { options->grid_refine_factors[2] = 1; }

        int nmesh_x, nmesh_y, nmesh_z;
        DOUBLE gridsize[3];
        int status = get_gridsize(&gridsize[0],
                                  &nmesh_x,
                                  xmax - xmin,
                                  xwrap,
                                  rmax,
        ...
        status += get_gridsize(&gridsize[1],
                               &nmesh_y,
                               ymax - ymin,
                               ywrap,
                               rmax,
                               ...
                               options->max_cells_per_dim);
        status += get_gridsize(&gridsize[2],
                               &nmesh_z,
                               zmax - zmin,
                               zwrap,
                               rmax,
        ...
        // Use the old heuristic of boosting the first two grid refs if the average
        // number of particles per cell is too high FUTURE lots of ways to improve this!
    ...
    cellarray *lattice1 = gridlink(ND1,
                                   X1,
                                   Y1,
                                   Z1,
                                   W1,
    ...
    if (lattice1 == NULL) { return EXIT_FAILURE; }

    cellarray *lattice2 = NULL;
    if (options->autocorr == 0) {
        lattice2 = gridlink(ND2,
                            X2,
                            Y2,
                            Z2,
                            W2,
        ...
        if (!(lattice1->nmesh_x == lattice2->nmesh_x
              && lattice1->nmesh_y == lattice2->nmesh_y
              && lattice1->nmesh_z == lattice2->nmesh_z)) {
            sprintf(
               ERRMSG,
               "Error: The two sets of 3-D lattices do not have identical bins. First has dims (%d, %d, %d) while second has (%d, %d, %d)\n",
               lattice1->nmesh_x,
               lattice1->nmesh_y,
               lattice1->nmesh_z,
               lattice2->nmesh_x,
    ...
    struct cell_pair *all_cell_pairs =
       generate_cell_pairs(&num_cell_pairs,
                           lattice1,
                           lattice2,
                           options->grid_refine_factors[0],
                           options->grid_refine_factors[1],
                           ...
                           rmax,
                           -1.0,
                           -1.0, /*max_3D_sep, max_2D_sep, max_1D_sep*/
                           enable_min_sep_opt,
                           options->autocorr,
                           periodic_x,
                           periodic_y,
    ...
    if (all_cell_pairs == NULL) {
        free_cellarray(&lattice1);
        if (options->autocorr == 0) { free_cellarray(&lattice2); }
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < N_bin_edges - 1; i++) {
        npairs[i] = 0;
        if (options->need_avg_sep) { ravg[i] = 0.0; }
        if (need_wavg) { wavg[i] = 0.0; }
    }

    // Initialize the progress bar
    int64_t numdone = 0;
    if (options->verbose) { init_my_progressbar(num_cell_pairs); }
...
#pragma omp parallel num_threads(options->numthreads)
#endif
    {
        ...
        local_wavg[tid] =
           need_wavg ? (DoubleAccum *) my_malloc(sizeof(DoubleAccum), N_bin_edges) :
                       NULL;
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) shared(numdone, interrupted) \
   num_threads(options->numthreads)
    ...
    for (int64_t icellpair = 0; icellpair < num_cell_pairs; icellpair++) {
        ...
        if (options->verbose) {
            if (tid == 0) { my_progressbar(numdone); }
#ifdef _OPENMP
#pragma omp atomic
#endif
            numdone++;
        ...
        countpairs_function(this_npairs,
                            this_ravg,
                            this_wavg,
                            first_N,
                            lattice1->X + i,
                            ...
                            N_bin_edges,
                            bin_edges_sqr,
                            this_cell_pair->xwrap,
                            this_cell_pair->ywrap,
                            this_cell_pair->zwrap,
                            this_cell_pair->min_dx,
                            this_cell_pair->min_dy,
                            this_cell_pair->min_dz,
                            this_cell_pair->closest_x1,
                            this_cell_pair->closest_y1,
                            this_cell_pair->closest_z1,
    ...
    // (2) there's no way to parallelize the reductions (only relevant for big
    // N_bin_edges); (3) reductions live on the stack, and I'm not sure we want to rely
    // on that for large arrays.
...
#pragma omp for schedule(static, CACHELINE / sizeof(int64_t))
        ...
        for (int j = 0; j < N_bin_edges - 1; j++) {
            for (int i = 0; i < options->numthreads; i++) {
                ...
                if (need_wavg) { wavg[j] += local_wavg[i][j + 1]; }
            }
        }
        ...
        if (options->need_avg_sep) { free(local_ravg[tid]); }
        if (need_wavg) { free(local_wavg[tid]); }
    }

    if (options->verbose) { finish_myprogressbar(); }
    ...
    if (options->autocorr == 1) {
        for (int i = 0; i < N_bin_edges - 1; i++) {
            npairs[i] *= 2;
            if (options->need_avg_sep) { ravg[i] *= (DOUBLE) 2.0; }
            if (need_wavg) { wavg[i] *= (DOUBLE) 2.0; }
        }
        ...
        if (bin_edges[0] <= 0.0) {
            ...
            // We don't need to add anything to ravg; all the self-pairs have 0
            // separation! The self-pairs have non-zero weight, though.  So, fix that
            // here.
            if (need_wavg) {
                // Keep in mind this is an autocorrelation (i.e. only one particle set
                // to consider)
                weight_func_t weight_func =
                   get_weight_func_by_method(options->weight_method);
    ...
    for (int i = 0; i < N_bin_edges - 1; i++) {
        if (npairs[i] > 0) {
            if (options->need_avg_sep) { ravg[i] /= (DoubleAccum) npairs[i]; }
            if (need_wavg) { wavg[i] /= (DoubleAccum) npairs[i]; }
        }
    }
