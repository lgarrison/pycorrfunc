...
#pragma GCC diagnostic ignored "-Wfloat-equal"
    ...
    if(xbinsize == ZERO && NPART > 2) warn_flat_dim("X");
    if(ybinsize == ZERO && NPART > 2) warn_flat_dim("Y");
    if(zbinsize == ZERO && NPART > 2) warn_flat_dim("Z");
...
#pragma GCC diagnostic pop
    ...
    cellarray *lattice  = (cellarray *) my_malloc(sizeof(*lattice), totncells);
