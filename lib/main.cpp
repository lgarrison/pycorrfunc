...
void countpairs_wrapper(
   ...
   int max_cells) {
        ...
        std::visit(
           [&options](auto &&arg) {
               if constexpr (std::is_same_v<int, std::decay_t<decltype(arg)>>) {
                   for (int i = 0; i < 3; i++) { options.grid_refine_factors[i] = arg; }
               } else {
                   auto arr = arg;
                   for (int i = 0; i < 3; i++) {
    ...
    if (status != EXIT_SUCCESS) {
        // FUTURE if countpairs was using C++, we could easily raise more informative
        // exceptions with proper contextual/chained error messages. For now, we'll use
        // the last message in the ERRMSG buffer.
        throw std::runtime_error(ERRMSG);
    }
}
