...
#define ASSIGN_CELL_TIMINGS(                                             \
   thread_timings, nx1, nx2, timediff, tid, first_cellid, second_cellid) \
    {                                                                    \
...
#define XRETURN(EXP, VAL, ...)                                                     \
    do {                                                                           \
        if (!(EXP)) {                                                              \
            int nwrite = sprintf(                                                  \
               ERRMSG,                                                             \
               "An internal error has occurred: %s:%d (%s) with expression `" #EXP \
               "'\n" ANSI_COLOR_BLUE                                               \
               "Please file an issue on GitHub." ANSI_COLOR_RESET "\n",            \
               __FILE__,                                                           \
               __LINE__,                                                           \
               __FUNCTION__);                                                      \
