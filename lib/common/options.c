...
int get_config_options(config_options *options, const char *weight_method) {
    ...
    if (get_weight_method_by_name(weight_method, &options->weight_method)
        != EXIT_SUCCESS) {
    ...
    /* Options for DDrppi_mocks (FAST_DIVIDE is also applicable for both DDsmu, and
     * DDsmu_mocks) */
...
void allocate_cell_timer(config_options *options, const int64_t num_cell_pairs) {
    ...
    options->cell_timings =
       (api_cell_timings *) calloc(num_cell_pairs, sizeof(*(options->cell_timings)));
...
void assign_cell_timer(api_cell_timings *cell_timings,
    ...
    /* This looks like a repeated "if" condition but it is not. Covers the case for the
     * calloc failure above */
    if (options->totncells_timings >= num_cell_pairs) {
        memmove(options->cell_timings,
                cell_timings,
                sizeof(api_cell_timings) * num_cell_pairs);
    }
}
...
void set_grid_refine_scheme(config_options *options, const int8_t flag) {
    ...
    options->gridding_flags =
       (options->gridding_flags & ~GRIDDING_REF_MASK) | (flag & GRIDDING_REF_MASK);
}
...
void set_grid_refine_factors(config_options *options,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (grid_refine_factors[i] > INT8_MAX) {
            fprintf(
               stderr,
               "Warning: grid refine factor[%d] can be at most %d. Found %d instead\n",
               i,
               INT8_MAX,
               grid_refine_factors[i]);
            grid_refine = 1;
        }
        options->grid_refine_factors[i] = grid_refine;
    }
    /*
      Note, programmatically setting the refine factors resets the gridding flag to
      "GRIDDING_DFL" GRIDDING_CUST is only set via function parameters, or explicitly
    */
