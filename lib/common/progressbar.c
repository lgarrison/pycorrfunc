// FUTURE: in theory, we could update a tqdm progress bar via nanobind. Would that hurt
// performance?
...
void init_my_progressbar(const int64_t N) {
    if (N <= 0) {
        fprintf(stderr,
                "WARNING: N=%" PRId64
                " is not positive. Progress bar will not be printed\n",
                N);
        SMALLPRINTSTEP = 0.0;
    } else {
        // set the increment
...
void my_progressbar(const int64_t curr_index) {
    if (SMALLPRINTSTEP > 0.0) {
        percent =
           (curr_index + 1) / SMALLPRINTSTEP;  // division is in double -> C has 0-based
                                               // indexing -- the +1 accounts for that.
        int integer_percent = (int) percent;

        if (integer_percent >= 0 && integer_percent <= 100) {
            /*        for(int
             * i=beg_of_string_index;i<END_INDEX_FOR_PERCENT_DONE[integer_percent];i++)
             */
            /*            fprintf(stderr,"%c",PROGRESSBARSTRING[i]); */

            fprintf(stderr,
                    "%.*s",
                    END_INDEX_FOR_PERCENT_DONE[integer_percent] - beg_of_string_index,
