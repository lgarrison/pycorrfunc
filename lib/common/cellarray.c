...
int validate_cellarray(const cellarray *lattice) {
    if (lattice->offsets == NULL || lattice->X == NULL || lattice->Y == NULL
        || lattice->Z == NULL) {
        return EXIT_FAILURE;
    }
    if (lattice->xbounds[0] == NULL || lattice->xbounds[1] == NULL
        || lattice->ybounds[0] == NULL || lattice->ybounds[1] == NULL
        || lattice->zbounds[0] == NULL || lattice->zbounds[1] == NULL) {
        return EXIT_FAILURE;
    }
    if (lattice->have_weights && lattice->W == NULL) { return EXIT_FAILURE; }
...
void free_cellarray(cellarray **lattice_ptr) {
    ...
    *lattice_ptr       = NULL;
    ...
    free(lattice->offsets);
    lattice->offsets = NULL;
    free(lattice->xbounds[0]);
    lattice->xbounds[0] = NULL;
    free(lattice->xbounds[1]);
    lattice->xbounds[1] = NULL;
    free(lattice->ybounds[0]);
    lattice->ybounds[0] = NULL;
    free(lattice->ybounds[1]);
    lattice->ybounds[1] = NULL;
    free(lattice->zbounds[0]);
    lattice->zbounds[0] = NULL;
    free(lattice->zbounds[1]);
    lattice->zbounds[1] = NULL;
