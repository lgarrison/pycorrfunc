...
// for computing ravg and weightavg
union int8 {
    AVX_INTS m_ibin;
    int ibin[AVX_NVEC];
};
...
#define CHECK_AND_FAST_DIVIDE_AVX(                                                     \
   result, numerator, denominator, fast_divide_and_NR_steps)                           \
    {                                                                                  \
        ...
        if (fast_divide_and_NR_steps == 0) {                                           \
            ...
            const __m128 float_tmp1 = _mm256_cvtpd_ps(                                 \
               denominator); /* convert double to float -> not avx_floats := _m256d */ \
            /*(convert 4 doubles into 4 floats -> use half of available 256 bit SIMD   \
             * registers) */                                                           \
            __m128 float_inv_tmp1 = _mm_rcp_ps(                                        \
               float_tmp1); /* intrinsic for 128 bit float approximate reciprocal */   \
            const AVX_FLOATS rc =                                                      \
               _mm256_cvtps_pd(float_inv_tmp1); /* convert back to double */           \
   ...
   result, numerator, denominator, fast_divide_and_NR_steps)                           \
    {                                                                                  \
        ...
        if (fast_divide_and_NR_steps == 0) {                                           \
            ...
            /* following blocks do an approximate reciprocal followed by two           \
             * iterations of Newton-Raphson */                                         \
            const AVX_FLOATS rc = _mm256_rcp_ps(                                       \
               denominator); /* intrinsic for 256 bit approximate reciprocal */        \
            /* We have the double->float->approx. reciprocal->double process done. */  \
            /* Now improve the accuracy of the divide with newton-raphson. */          \
            const AVX_FLOATS two = AVX_SET_FLOAT((DOUBLE) 2.0);                        \
            AVX_FLOATS rc_iter   = rc;                                                 \
