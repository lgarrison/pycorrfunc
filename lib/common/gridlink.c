...
cellarray *gridlink(const int64_t NPART,
                    DOUBLE *const X,
                    DOUBLE *const Y,
                    DOUBLE *const Z,
                    DOUBLE *const W,
    ...
    const int xstatus = get_gridsize(&xgridsize,
                                     &nmesh_x,
                                     xmax - xmin,
                                     xwrap,
                                     max_x_size,
    ...
    const int ystatus = get_gridsize(&ygridsize,
                                     &nmesh_y,
                                     ymax - ymin,
                                     ywrap,
                                     max_y_size,
                                     ygrid_refine_factor,
                                     options->max_cells_per_dim);
    const int zstatus = get_gridsize(&zgridsize,
                                     &nmesh_z,
                                     zmax - zmin,
                                     zwrap,
                                     max_z_size,
    ...
    cellarray *lattice =
       allocate_cellarray(NPART, nmesh_x, nmesh_y, nmesh_z, W != NULL);
    if (lattice == NULL) {
...
#pragma omp parallel for schedule(static) reduction(+ : out_of_bounds) \
   num_threads(options->numthreads)
    ...
    for (int64_t i = 0; i < NPART; i++) {
        ...
        out_of_bounds += ix < 0 || ix >= nmesh_x || iy < 0 || iy >= nmesh_y || iz < 0
                         || iz >= nmesh_z;
    ...
    if (out_of_bounds != 0) {
        sprintf(ERRMSG,
                "Error: %" PRId64
                " particles are out of bounds. Check periodic wrapping?\n",
                out_of_bounds);
    ...
    // Now determine the number of particles per cell so each cell knows its global
    // starting index This information is used for both the copy and in-place versions
    int nthreads = omp_get_max_threads();  // this is the upper bound, we will find the
                                           // actual value (usually the same) below
...
#pragma omp parallel num_threads(options->numthreads)
#endif
    {
        ...
        nthreads = omp_get_num_threads();  // reduce to actual number of threads, will
                                           // be the same in almost all cases
...
#pragma omp parallel num_threads(options->numthreads)
    ...
    if (nwritten == NULL) {
        ...
        sprintf(
           ERRMSG,
           "Error: Could not allocate memory for storing the number of particles per cell\n");
        return NULL;
    }
...
#pragma omp parallel for schedule(static) num_threads(options->numthreads)
#endif
    for (int64_t c = 0; c < tot_ncells; c++) { nwritten[c] = 0; }
...
#pragma omp parallel num_threads(options->numthreads)
#endif
    {
        ...
        // Second loop over particles: having computed the starts of each cell, fill the
        // cells. Each thread is going to loop over all particles, but only process
        // those in its cell domain.
        for (int64_t i = 0; i < NPART; i++) {
            const int64_t icell = all_cell_indices[i];
            if (icell < cstart || icell >= cend) { continue; }
    ...
    for (int64_t c = 0; c < tot_ncells; c++) {
        if (nwritten[c] != lattice->offsets[c + 1] - lattice->offsets[c]) {
            sprintf(
               ERRMSG,
               "Error: Cell %ld has %ld particles, but should have %ld particles\n",
               c,
               nwritten[c],
               lattice->offsets[c + 1] - lattice->offsets[c]);
            free(all_cell_indices);
            free(nwritten);
            free_cellarray(&lattice);
            return NULL;
...
struct cell_pair *generate_cell_pairs(int64_t *ncell_pairs,
                                      ...
                                      const DOUBLE max_2D_sep,
                                      const DOUBLE max_1D_sep,
    ...
    XRETURN(all_cell_pairs != NULL,
            ...
            "Reducing grid refine factors might help. Requested for %" PRId64
            " elements "
            "with each element of size %zu bytes\n",
            max_num_cell_pairs,
            sizeof(*all_cell_pairs));

    /* Under periodic boundary conditions + small nmesh_x/y/z, the periodic wrapping
       would cause the same cell to be included as an neighbour cell from both the left
       and the right side (i.e., when including cells with -grid_refine_factor, and when
       including cells up to +grid_refine_factor)

       Previously this would throw an error, but we can simply not add the duplicate
       cells.
       ...
       MS: 23/8/2019
     */
    const int check_for_duplicate_ngb_cells =
       (any_periodic
        && (nmesh_x < (2 * xgrid_refine_factor + 1)
            || nmesh_y < (2 * ygrid_refine_factor + 1)
            || nmesh_z < (2 * zgrid_refine_factor + 1))) ?
          1 :
          0;
    for (int64_t icell = 0; icell < lattice1->tot_ncells; icell++) {
        ...
        for (int iix = -xgrid_refine_factor; iix <= xgrid_refine_factor; iix++) {
            ...
            const DOUBLE off_xwrap = ((ix + iix) >= 0) && ((ix + iix) < nmesh_x) ?
                                        0.0 :
                                        ((ix + iix) < 0 ? xwrap : -xwrap);
            ...
            for (int iiy = -ygrid_refine_factor; iiy <= ygrid_refine_factor; iiy++) {
                ...
                const DOUBLE off_ywrap = ((iy + iiy) >= 0) && ((iy + iiy) < nmesh_y) ?
                                            0.0 :
                                            ((iy + iiy) < 0 ? ywrap : -ywrap);
                for (int64_t iiz = -zgrid_refine_factor; iiz <= zgrid_refine_factor;
                     iiz++) {
                    ...
                    const DOUBLE off_zwrap =
                       ((iz + iiz) >= 0) && ((iz + iiz) < nmesh_z) ?
                          0.0 :
                          ((iz + iiz) < 0 ? zwrap : -zwrap);
                    ...
                    int64_t second_N =
                       lattice2->offsets[icell2 + 1] - lattice2->offsets[icell2];
                    if ((autocorr == 1 && icell2 > icell) || second_N == 0) {
                        continue;
                    }

                    // Check if the ngb-cell has already been added - can happen under
                    // periodic boundary conditions, with small value of nmesh_x/y/z (ie
                    // large Rmax relative to BoxSize)
                    ...
                    if (enable_min_sep_opt) {
                        ...
                        const DOUBLE x_low = lattice1->xbounds[0][icell] + off_xwrap,
                                     x_hi  = lattice1->xbounds[1][icell] + off_xwrap;
                        const DOUBLE y_low = lattice1->ybounds[0][icell] + off_ywrap,
                                     y_hi  = lattice1->ybounds[1][icell] + off_ywrap;
                        const DOUBLE z_low = lattice1->zbounds[0][icell] + off_zwrap,
                                     z_hi  = lattice1->zbounds[1][icell] + off_zwrap;
                        ...
                        const DOUBLE second_x =
                           iix < 0 ?
                              lattice2->xbounds[1][icell2] :
                              lattice2->xbounds[0][icell2];  // second condition also
                                                             // contains iix==0
                        ...
                        const DOUBLE first_y =
                           iiy < 0 ? y_low :
                                     y_hi;  // second condition also contains iix==0
                        const DOUBLE second_y =
                           iiy < 0 ?
                              lattice2->ybounds[1][icell2] :
                              lattice2->ybounds[0][icell2];  // second condition also
                                                             // contains iix==0
                        ...
                        const DOUBLE first_z =
                           iiz < 0 ? z_low :
                                     z_hi;  // second condition also contains iix==0
                        const DOUBLE second_z =
                           iiz < 0 ?
                              lattice2->zbounds[1][icell2] :
                              lattice2->zbounds[0][icell2];  // second condition also
                                                             // contains iix==0
                        ...
                        /* in a separate if (rather than an else with the previous if
                           for max_3D_sep) so that conditions can be chained */
                        if (max_2D_sep > 0) {
                            const DOUBLE sqr_min_sep_cells =
                               min_dx * min_dx + min_dy * min_dy;
