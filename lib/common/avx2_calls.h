...
#define AVX2_MASKSTORE_FLOATS(dest, mask, source) \
    _mm256_maskstore_ps(dest, mask, source)
...
#define AVX2_MASKSTORE_FLOATS(dest, mask, source) \
    _mm256_maskstore_pd(dest, mask, source)

// Trig
#ifdef __INTEL_COMPILER
#define AVX2_ARC_COSINE(X, order) _mm256_acos_pd(X)
#else
#define AVX2_ARC_COSINE(X, order)                                     \
    inv_cosine_avx(X, order)  // avx and avx2 are the same wrt floats
#endif
