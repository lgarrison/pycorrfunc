...
/*
  ...
  -> print_time will output `Time taken to execute `do something' = 3 hours 40 mins 30.1
  seconds


  (code can be easily extended to include `weeks' as a system of time unit. left to the
  reader)
*/
    ...
    if (timediff < ratios[2]) {
        ...
        while (which < 4) {
            ...
            if (time_to_print > 1) {
                ...
                my_snprintf(
                   tmp, MAXLINESIZE, "%5d %s", (int) time_to_print, units[which]);
...
void *my_malloc(size_t size, int64_t N) {
    ...
    if (x == NULL) {
        sprintf(ERRMSG,
                "malloc for %" PRId64 " elements with %zu bytes failed...\n",
                N,
                size);
        perror(NULL);
    }
...
void *my_calloc(size_t size, int64_t N) {
    ...
    if (x == NULL) {
        sprintf(ERRMSG,
                "malloc for %" PRId64 " elements with %zu size failed...\n",
                N,
                size);
        perror(NULL);
    }
...
void my_free(void **x) {
    ...
    */
    ...
    if (*x != NULL) free(*x);  // free the memory
...
/* A parallel cumulative sum
   ...
   - compute the "offset" for each chunk by summing the cumsum at the tail of all
   previous chunks
   - apply the offset
*/
void parallel_cumsum(const int64_t *a, const int64_t N, int64_t *cumsum) {
    ...
    if (N / min_N_per_thread < nthreads) { nthreads = N / min_N_per_thread; }
...
#pragma omp barrier
        ...
        int64_t offset = 0;
        for (int t = 0; t < tid; t++) { offset += cumsum[N * (t + 1) / nthreads - 1]; }
...
#pragma omp barrier
        ...
        if (offset != 0) {
            for (int64_t c = cstart; c < cend; c++) { cumsum[c] += offset; }
        }
    }
}
