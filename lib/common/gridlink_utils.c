...
int get_gridsize(DOUBLE *xgridsize,
                 int *nlattice,
                 const DOUBLE xdiff,
                 const DOUBLE xwrap,
                 const DOUBLE rmax,
    ...
    if (xwrap > 0 && rmax >= xwrap / 2) {
        sprintf(
           ERRMSG,
           "%s> ERROR: rmax=%f must be less than half of periodic boxize=%f to avoid double-counting particles\n",
           __FILE__,
           rmax,
           xwrap);
        return EXIT_FAILURE;
...
void get_max_min(const int64_t ND1,
...
#ifdef _OPENMP
#pragma omp parallel for reduction(min : xmin, ymin, zmin) \
   reduction(max : xmax, ymax, zmax)
#endif
...
void get_max_min_ra_dec(const int64_t ND1,
                        const DOUBLE *RA,
                        const DOUBLE *DEC,
                        DOUBLE *ra_min,
                        DOUBLE *dec_min,
...
DOUBLE find_closest_pos(const DOUBLE first_xbounds[2],
...
void find_min_and_max_sqr_sep_between_cell_pairs(const DOUBLE first_xbounds[2],
    ...
    if (first_xbounds[0] > second_xbounds[1])
        min_sqr_sep += (first_xbounds[0] - second_xbounds[1])
                       * (first_xbounds[0] - second_xbounds[1]);
    if (second_xbounds[0] > first_xbounds[1])
        min_sqr_sep += (second_xbounds[0] - first_xbounds[1])
                       * (second_xbounds[0] - first_xbounds[1]);

    if (first_ybounds[0] > second_ybounds[1])
        min_sqr_sep += (first_ybounds[0] - second_ybounds[1])
                       * (first_ybounds[0] - second_ybounds[1]);
    if (second_ybounds[0] > first_ybounds[1])
        min_sqr_sep += (second_ybounds[0] - first_ybounds[1])
                       * (second_ybounds[0] - first_ybounds[1]);

    if (first_zbounds[0] > second_zbounds[1])
        min_sqr_sep += (first_zbounds[0] - second_zbounds[1])
                       * (first_zbounds[0] - second_zbounds[1]);
    if (second_zbounds[0] > first_zbounds[1])
        min_sqr_sep += (second_zbounds[0] - first_zbounds[1])
                       * (second_zbounds[0] - first_zbounds[1]);
    ...
    const DOUBLE max_sqr_sep = (xmax - xmin) * (xmax - xmin)
                               + (ymax - ymin) * (ymax - ymin)
                               + (zmax - zmin) * (zmax - zmin);
