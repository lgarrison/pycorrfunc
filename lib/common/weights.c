...
inline int get_weight_method_by_name(const char *name, weight_method_t *method) {
    ...
    // These should not be strncmp because we want the implicit length comparison of
    // strcmp. It is still safe because one of the args is a string literal.
...
weight_func_t get_weight_func_by_method(const weight_method_t method) {
    switch (method) {
        case PAIR_PRODUCT: return &pair_product;
        default:
