...
#define AVX512_MASK_ADD_INTS(FALSEVALS, MASK, X, Y) \
    _mm512_mask_add_epi32(FALSEVALS, MASK, X, Y)
...
#define AVX512_MASKZ_MULTIPLY_INTS_LOW32(MASK, X, Y) \
    _mm512_maskz_mullo_epi32(MASK, X, Y)
...
#define AVX512_MASK_LOAD_FLOATS_ALIGNED(FALSEVALS, MASK, X) \
    _mm512_mask_load_ps(FALSEVALS, MASK, X)
...
#define AVX512_MASK_MULTIPLY_FLOATS(FALSEVALS, MASK, X, Y) \
    _mm512_mask_mul_ps(FALSEVALS, MASK, X, Y)
...
#define AVX512_MASK_DIVIDE_FLOATS(FALSEVALS, MASK, X, Y) \
    _mm512_mask_div_ps(FALSEVALS, MASK, X, Y)
#define AVX512_MASKZ_DIVIDE_FLOATS(MASK, X, Y) _mm512_maskz_div_ps(MASK, X, Y)

#define AVX512_ADD_FLOATS(X, Y) _mm512_add_ps(X, Y)
#define AVX512_MASK_ADD_FLOATS(FALSEVALS, MASK, X, Y) \
    _mm512_mask_add_ps(FALSEVALS, MASK, X, Y)
#define AVX512_MASKZ_ADD_FLOATS(MASK, X, Y) _mm512_maskz_add_ps(MASK, X, Y)

#define AVX512_SUBTRACT_FLOATS(X, Y) _mm512_sub_ps(X, Y)
#define AVX512_MASK_SUBTRACT_FLOATS(FALSEVALS, MASK, X, Y) \
    _mm512_mask_sub_ps(FALSEVALS, MASK, X, Y)
...
#define AVX512_MASKZ_FNMA_ADD_FLOATS(X, MASK, Y, Z) \
    _mm512_maskz_fnmadd_ps(MASK, X, Y, Z)

/* returns XY - Z */
#define AVX512_FMA_SUBTRACT_FLOATS(X, Y, Z) _mm512_fmsub_ps(X, Y, Z)
#define AVX512_MASK_FMA_SUBTRACT_FLOATS(X, MASK, Y, Z) \
    _mm512_mask_fmsub_ps(X, MASK, Y, Z)
#define AVX512_MASKZ_FMA_SUBTRACT_FLOATS(X, MASK, Y, Z) \
    _mm512_maskz_fmsub_ps(MASK, X, Y, Z)
...
#define AVX512_MASK_SQRT_FLOAT(FALSEVALS, MASK, X) \
    _mm512_mask_sqrt_ps(FALSEVALS, MASK, X)
...
#define AVX512_MASK_RECIPROCAL_FLOATS(FALSEVALS, MASK, X) \
    _mm512_mask_rcp14_ps(FALSEVALS, MASK, X)
...
#define AVX512_BLEND_FLOATS_WITH_MASK(MASK, FALSE, TRUE) \
    _mm512_mask_blend_ps(MASK, FALSE, TRUE)
#define AVX512_BLEND_INTS_WITH_MASK(MASK, FALSE, TRUE) \
    _mm512_mask_blend_epi32(MASK, FALSE, TRUE)
...
#define AVX512_MASK_ADD_INTS(FALSEVALS, MASK, X, Y) \
    _mm256_mask_add_epi32(FALSEVALS, MASK, X, Y)
...
#define AVX512_MASKZ_MULTIPLY_INTS_LOW32(MASK, X, Y) \
    _mm256_maskz_mullo_epi32(MASK, X, Y)

#elif defined(__AVX512F__)
#define AVX512_MASK_ADD_INTS(FALSEVALS, MASK, X, Y)                                 \
    _mm512_castsi512_si256(_mm512_mask_add_epi32(_mm512_castsi256_si512(FALSEVALS), \
                                                 MASK,                              \
                                                 _mm512_castsi256_si512(X),         \
                                                 _mm512_castsi256_si512(Y)))
#define AVX512_MASKZ_ADD_INTS(MASK, X, Y)                           \
    _mm512_castsi512_si256(_mm512_maskz_add_epi32(                  \
       MASK, _mm512_castsi256_si512(X), _mm512_castsi256_si512(Y)))

#define AVX512_MASK_MULTIPLY_INTS(FALSEVALS, MASK, X, Y)                            \
    _mm512_castsi512_si256(_mm512_mask_mul_epi32(_mm512_castsi256_si512(FALSEVALS), \
                                                 MASK,                              \
                                                 _mm512_castsi256_si512(X),         \
                                                 _mm512_castsi256_si512(Y)))
#define AVX512_MASKZ_MULTIPLY_INTS(MASK, X, Y)                      \
    _mm512_castsi512_si256(_mm512_maskz_mul_epi32(                  \
       MASK, _mm512_castsi256_si512(X), _mm512_castsi256_si512(Y)))

#define AVX512_MASKZ_MULTIPLY_INTS_LOW32(MASK, X, Y)                \
    _mm512_castsi512_si256(_mm512_maskz_mullo_epi32(                \
       MASK, _mm512_castsi256_si512(X), _mm512_castsi256_si512(Y)))

#endif
#endif /* commenting out the int math operations since they are either cumbersome or \
          produce results of different SIMD widths*/
...
#define AVX512_MASK_LOAD_FLOATS_ALIGNED(FALSEVALS, MASK, X) \
    _mm512_mask_load_pd(FALSEVALS, MASK, X)
...
#define AVX512_MASK_MULTIPLY_FLOATS(FALSEVALS, MASK, X, Y) \
    _mm512_mask_mul_pd(FALSEVALS, MASK, X, Y)
...
#define AVX512_MASKZ_FNMA_ADD_FLOATS(X, MASK, Y, Z) \
    _mm512_maskz_fnmadd_pd(MASK, X, Y, Z)

/* returns XY - Z */
#define AVX512_FMA_SUBTRACT_FLOATS(X, Y, Z) _mm512_fmsub_pd(X, Y, Z)
#define AVX512_MASK_FMA_SUBTRACT_FLOATS(X, MASK, Y, Z) \
    _mm512_mask_fmsub_pd(X, MASK, Y, Z)
#define AVX512_MASKZ_FMA_SUBTRACT_FLOATS(X, MASK, Y, Z) \
    _mm512_maskz_fmsub_pd(MASK, X, Y, Z)
...
#define AVX512_MASK_SQRT_FLOAT(FALSEVALS, MASK, X) \
    _mm512_mask_sqrt_pd(FALSEVALS, MASK, X)
...
#define AVX512_MASK_RECIPROCAL_FLOATS(FALSEVALS, MASK, X) \
    _mm512_mask_rcp14_pd(FALSEVALS, MASK, X)
...
#if defined(__AVX512VL__)
#define AVX512_BLEND_INTS_WITH_MASK(MASK, FALSE, TRUE) \
    _mm256_mask_blend_epi32(MASK, FALSE, TRUE)
#elif defined(__AVX2__)
#define AVX512_BLEND_INTS_WITH_MASK(MASK, FALSE, TRUE) \
    _mm256_blend_epi32(FALSE, TRUE, MASK)  // AVX2
#else
#define AVX512_BLEND_INTS_WITH_MASK(MASK, FALSE, TRUE) \
    _blend_epi32_with_mask(MASK, FALSE, TRUE)
#endif
...
#define CHECK_AND_FAST_DIVIDE_AVX512(                                                 \
   result, numerator, denominator, mask, fast_divide_and_NR_steps)                    \
    {                                                                                 \
        /* For double precision floats */                                             \
        if (fast_divide_and_NR_steps == 0) {                                          \
            result = AVX512_MASKZ_DIVIDE_FLOATS(mask, numerator, denominator);        \
        } else {                                                                      \
            /* following blocks do an approximate reciprocal followed by two          \
             * iterations of Newton-Raphson */                                        \
            const AVX512_FLOATS rc =                                                  \
               AVX512_MASKZ_RECIPROCAL_FLOATS(mask, denominator);                     \
            ...
            for (unsigned int _ii = 0; _ii < fast_divide_and_NR_steps; _ii++) {       \
                rc_iter = AVX512_MULTIPLY_FLOATS(                                     \
                   rc_iter,                                                           \
                   AVX512_FNMA_ADD_FLOATS(                                            \
                      denominator, rc_iter, two)); /*2.0 - l^2*rc */                  \
            }                                                                         \
