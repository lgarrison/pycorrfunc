...
cellarray_mocks *gridlink_mocks(const int64_t NPART,
    ...
    struct timeval t0;
    if (options->verbose) { gettimeofday(&t0, NULL); }

    DOUBLE xbinsize = ZERO, ybinsize = ZERO, zbinsize = ZERO;
    DOUBLE wrap       = ZERO;
    const int xstatus = get_binsize(xmax - xmin,
                                    wrap,
                                    max_x_size,
                                    xbin_refine_factor,
                                    ...
                                    &xbinsize,
                                    &nmesh_x);
    const int ystatus = get_binsize(ymax - ymin,
                                    wrap,
                                    max_y_size,
                                    ybin_refine_factor,
                                    options->max_cells_per_dim,
                                    &ybinsize,
                                    &nmesh_y);
    const int zstatus = get_binsize(zmax - zmin,
                                    wrap,
                                    max_z_size,
                                    zbin_refine_factor,
                                    options->max_cells_per_dim,
    ...
    if (xstatus != EXIT_SUCCESS || ystatus != EXIT_SUCCESS || zstatus != EXIT_SUCCESS) {
        fprintf(stderr,
                "Received xstatus = %d ystatus = %d zstatus = %d. Error\n",
                xstatus,
                ystatus,
                zstatus);
        return NULL;
    ...
    int64_t expected_n =
       (int64_t) (NPART * cell_volume / box_volume * MEMORY_INCREASE_FAC);
    expected_n = expected_n < 2 ? 2 : expected_n;

    if (options->verbose) {
        fprintf(stderr,
                "In %s> Running with [nmesh_x, nmesh_y, nmesh_z]  = %d,%d,%d. ",
                __FUNCTION__,
                nmesh_x,
                nmesh_y,
                nmesh_z);
    ...
    cellarray_mocks *lattice =
       (cellarray_mocks *) my_malloc(sizeof(*lattice), totncells);
    int64_t *all_cell_indices = NULL;
    int64_t *original_indices = NULL;
    int64_t *nallocated =
       NULL;  // to keep track of how many particles have been allocated per cell (only
              // when creating a copy of particle positions)
    ...
    if (lattice == NULL || (options->copy_particles == 0 && all_cell_indices == NULL)
        ...
        fprintf(
           stderr,
           "Error: In %s> Could not allocate memory for creating the lattice and associated arrays\n",
           __FUNCTION__);
        return NULL;
    }
    ...
    for (int64_t icell = 0; icell < totncells; icell++) {
        ...
        lattice[icell].weights.num_weights =
           (WEIGHTS == NULL) ? 0 : WEIGHTS->num_weights;
        ...
        if (options->copy_particles) {
            ...
            lattice[icell].original_index =
               my_malloc(sizeof(lattice[icell].original_index[0]), expected_n);
            XRETURN(lattice[icell].original_index != NULL,
                    NULL,
                    "Error in %s> Could not allocate memory for holding the original "
            ...
            lattice[icell].x =
               my_malloc(memsize, expected_n);  // This allocates extra and is wasteful
            lattice[icell].y =
               my_malloc(memsize, expected_n);  // This allocates extra and is wasteful
            lattice[icell].z =
               my_malloc(memsize, expected_n);  // This allocates extra and is wasteful
            ...
            for (int w = 0; w < lattice[icell].weights.num_weights; w++) {
                lattice[icell].weights.weights[w] =
                   (DOUBLE *) my_malloc(memsize, expected_n);
            ...
            if (lattice[icell].x == NULL || lattice[icell].y == NULL
                || lattice[icell].z == NULL || w_alloc_status == EXIT_FAILURE) {
                for (int64_t j = icell; j >= 0; j--) {
                    free(lattice[j].x);
                    free(lattice[j].y);
                    free(lattice[j].z);
                    free(lattice[j].original_index);
                    for (int w = 0; w < lattice[icell].weights.num_weights; w++) {
    ...
    for (int64_t i = 0; i < NPART; i++) {
        ...
        if (iy > nmesh_y - 1) iy--;
        if (iz > nmesh_z - 1) iz--;
        XRETURN(X[i] >= xmin && X[i] <= xmax,
                NULL,
                "x[%" PRId64 "] = %" REAL_FORMAT " must be within [%" REAL_FORMAT
                ",%" REAL_FORMAT "]\n",
                i,
                X[i],
                xmin,
                xmax);
        XRETURN(Y[i] >= ymin && Y[i] <= ymax,
                NULL,
                "y[%" PRId64 "] = %" REAL_FORMAT " must be within [%" REAL_FORMAT
                ",%" REAL_FORMAT "]\n",
                i,
                Y[i],
                ymin,
                ymax);
        XRETURN(Z[i] >= zmin && Z[i] <= zmax,
                NULL,
                "z[%" PRId64 "] = %" REAL_FORMAT " must be within [%" REAL_FORMAT
                ",%" REAL_FORMAT "]\n",
                i,
                Z[i],
                zmin,
        ...
        XRETURN(
           ix >= 0 && ix < nmesh_x, NULL, "ix=%d must be within [0,%d)\n", ix, nmesh_x);
        XRETURN(
           iy >= 0 && iy < nmesh_y, NULL, "iy=%d must be within [0,%d)\n", iy, nmesh_y);
        XRETURN(
           iz >= 0 && iz < nmesh_z, NULL, "iz=%d must be within [0,%d)\n", iz, nmesh_z);

        const int64_t icell =
           CONVERT_3D_INDEX_TO_LINEAR(ix, iy, iz, nmesh_x, nmesh_y, nmesh_z);
        if (options->copy_particles == 0) {
            ...
            if (lattice[icell].nelements == nallocated[icell]) {
                ...
                while (expected_n <= nallocated[icell]) expected_n++;
                ...
                do {
                    posx =
                       my_realloc(lattice[icell].x, memsize, expected_n, "lattice.x");
                    posy =
                       my_realloc(lattice[icell].y, memsize, expected_n, "lattice.y");
                    posz =
                       my_realloc(lattice[icell].z, memsize, expected_n, "lattice.z");
                    orig_index = my_realloc(lattice[icell].original_index,
                                            sizeof(*(lattice[icell].original_index)),
                                            expected_n,
                                            "lattice.original_index");
                    ...
                    lattice[icell].original_index = (orig_index == NULL) ?
                                                       lattice[icell].original_index :
                                                       orig_index;
                    ...
                    for (int w = 0; w < lattice[icell].weights.num_weights; w++) {
                        DOUBLE *newweights =
                           (DOUBLE *) my_realloc(lattice[icell].weights.weights[w],
                                                 memsize,
                                                 expected_n,
                                                 "lattice.weights");
                        if (newweights == NULL) {
                    ...
                    if (posx == NULL || posy == NULL || posz == NULL
                        || orig_index == NULL || w_alloc_status == EXIT_FAILURE) {
                        expected_n--;
                    }
                ...
                if (expected_n == nallocated[icell]) {
                    ...
                    fprintf(
                       stderr,
                       "In %s> Reallocation failed,  randomly subsampling the input particle set (currently at %" PRId64
                       " particles) might help\n",
                       __FUNCTION__,
                       NPART);
                    fprintf(
                       stderr, "posx = %p posy = %p posz = %p \n", posx, posy, posz);
            ...
            XRETURN(lattice[icell].nelements < nallocated[icell],
                    NULL,
                    ANSI_COLOR_RED
                    "BUG: lattice[%" PRId64 "].nelements = %" PRId64
                    " must be less than allocated memory = %" PRId64 ANSI_COLOR_RESET
                    "\n",
                    icell,
            ...
            for (int w = 0; w < lattice[icell].weights.num_weights; w++) {
                lattice[icell].weights.weights[w][ipos] =
                   ((DOUBLE *) (WEIGHTS->weights[w]))[i];
            }
        }
        lattice[icell].xbounds[0] =
           X[i] < lattice[icell].xbounds[0] ? X[i] : lattice[icell].xbounds[0];
        lattice[icell].ybounds[0] =
           Y[i] < lattice[icell].ybounds[0] ? Y[i] : lattice[icell].ybounds[0];
        lattice[icell].zbounds[0] =
           Z[i] < lattice[icell].zbounds[0] ? Z[i] : lattice[icell].zbounds[0];

        lattice[icell].xbounds[1] =
           X[i] > lattice[icell].xbounds[1] ? X[i] : lattice[icell].xbounds[1];
        lattice[icell].ybounds[1] =
           Y[i] > lattice[icell].ybounds[1] ? Y[i] : lattice[icell].ybounds[1];
        lattice[icell].zbounds[1] =
           Z[i] > lattice[icell].zbounds[1] ? Z[i] : lattice[icell].zbounds[1];

        lattice[icell].nelements++;
    }

    if (options->copy_particles) {
        // All the particle positions have already been copied -> do not need to
        // re-allocate any more You can free the extra memory reserved by the mallocs by
        // looping over totncells and doing a
        // realloc(lattice[cellindex].x,sizeof(DOUBLE),lattice[cellindex].nelements,"lattice.x")
        free(nallocated);
    } else {
        // We have been told to work with the particle positions in-place i.e., not
        // create a copy of the particle positions within the lattice. Therefore, now we
        // have to sort the input particle positions to get them to be contiguous in
        // their respective 3D cell
        if (sizeof(*(lattice->original_index)) != sizeof(*original_indices)) {
            fprintf(
               stderr,
               "Error: In %s> The array to track the indices of input particle positions "
               "should be the same size as the indices themselves\n",
               __FUNCTION__);
            fprintf(stderr,
                    "Perhaps check that these two variables are the same type\n");
            fprintf(
               stderr,
               "'original_index' within the 'cellarray', defined in 'cellarray.h' and \n");
            fprintf(stderr,
                    "'original_indices' defined within function '%s' in file '%s'\n",
                    __FUNCTION__,
                    __FILE__);
            return NULL;
        }
    ...
    {                                                                    \
        ...
        for (int w = 0; w < WEIGHTS->num_weights; w++) {                 \
            SGLIB_ARRAY_ELEMENTS_EXCHANGER(                              \
               DOUBLE, ((DOUBLE *) WEIGHTS->weights[w]), i, j);          \
        }                                                                \
    }

        // If the input-array is sorted exactly, then the quicksort will become a
        // very-slow O(N^2) Try to protect the user.
        int64_t num_sorted =
           1;  // an array containing exactly one element is always sorted
        ...
        // If the input array is already sorted, then all_cell_indices will also be
        // sorted which would result in num_sorted == NPART
        if (num_sorted < NPART) {
            // Since the particles might be coming from an already sorted array -
            // quicksort might degenerate to a O(N^2) process -- heap-sort might be
            // safer.
            if (options->use_heap_sort
                || num_sorted > FRACTION_SORTED_REQD_TO_HEAP_SORT * NPART) {
                SGLIB_ARRAY_HEAP_SORT(int64_t,
                                      all_cell_indices,
                                      NPART,
                                      SGLIB_NUMERIC_COMPARATOR,
                                      MULTIPLE_ARRAY_EXCHANGER);
            } else {
                SGLIB_ARRAY_QUICK_SORT(int64_t,
                                       all_cell_indices,
                                       NPART,
                                       SGLIB_NUMERIC_COMPARATOR,
                                       MULTIPLE_ARRAY_EXCHANGER);
        ...
        // Now the particles are sorted contiguously according to their respective
        // cellindex We need to fix up the x/y/z pointers at the beginning of each cell
...
#undef MULTIPLE_ARRAY_EXCHANGER
        ...
        for (int64_t icell = 0; icell < totncells; icell++) {
            ...
            first->x =
               X + nelements_so_far;  // take the base pointer address and add however
                                      // many particles that have appeared summed across
                                      // all previous cells
            first->y              = Y + nelements_so_far;
            first->z              = Z + nelements_so_far;
            first->original_index = original_indices + nelements_so_far;
            for (int w = 0; w < WEIGHTS->num_weights; w++) {
                first->weights.weights[w] =
                   ((DOUBLE *) WEIGHTS->weights[w]) + nelements_so_far;
            }
            nelements_so_far += first->nelements;
        }
        XRETURN(nelements_so_far == NPART,
                NULL,
                "Error in %s> Expected to assign all particles = %" PRId64
                " into cells but only seem "
                "to have assigned %" PRId64
                ". Perhaps, there are some edge cases with floating point accuracy\n",
                __FUNCTION__,
                NPART,
                nelements_so_far);
    ...
    {                                                                                \
        ...
        for (int w = 0; w < first->weights.num_weights; w++) {                       \
            ...
            for (int64_t ii = 0; ii < first->nelements - 1; ii++) {
                // increment by 1 if the next element is greater than or equal to
                // current decrement by 1 if the next element is smaller
                num_sorted += (first->z[ii + 1] >= first->z[ii]) ? +1 : -1;
            }

            // Check if there is any sorting to do
            // If the input array is already sorted, then all_cell_indices will also be
            // sorted which would result in num_sorted == first->nelements
            if (num_sorted < first->nelements) {
                ...
                if (options->use_heap_sort
                    || num_sorted
                          > FRACTION_SORTED_REQD_TO_HEAP_SORT * first->nelements) {
                    SGLIB_ARRAY_HEAP_SORT(DOUBLE,
                                          first->z,
                                          first->nelements,
                                          SGLIB_NUMERIC_COMPARATOR,
                                          MULTIPLE_ARRAY_EXCHANGER);
                } else {
                    SGLIB_ARRAY_QUICK_SORT(DOUBLE,
                                           first->z,
                                           first->nelements,
                                           SGLIB_NUMERIC_COMPARATOR,
                                           MULTIPLE_ARRAY_EXCHANGER);
...
#undef MULTIPLE_ARRAY_EXCHANGER
    ...
    // and doing a
    // realloc(lattice[icell].x,sizeof(DOUBLE),lattice[icell].nelements,"lattice.x")
    *nlattice_x = nmesh_x;
    *nlattice_y = nmesh_y;
    *nlattice_z = nmesh_z;
    if (options->verbose) {
...
struct cell_pair *generate_cell_pairs_mocks(cellarray_mocks *lattice1,
    ...
    if (!(autocorr == 0 || autocorr == 1)) {
        fprintf(
           stderr,
           "Error: Strange value of autocorr = %d. Expected to receive either 1 (auto-correlations) or 0 (cross-correlations)\n",
           autocorr);
        return NULL;
    }
    ...
    struct cell_pair *all_cell_pairs =
       my_malloc(sizeof(*all_cell_pairs), max_num_cell_pairs);
    XRETURN(all_cell_pairs != NULL,
            NULL,
            "Error: Could not allocate memory for storing all the cell pairs. "
            "Reducing bin refine factors might help. Requested for %" PRId64
            " elements "
            "with each element of size %zu bytes\n",
            max_num_cell_pairs,
            sizeof(*all_cell_pairs));
    ...
    for (int64_t icell = 0; icell < totncells; icell++) {
        ...
        XRETURN(icell == (ix * nmesh_y * nmesh_z + iy * nmesh_z + (int64_t) iz),
                NULL,
                ANSI_COLOR_RED "BUG: Index reconstruction is wrong. icell = %" PRId64
                               " reconstructed index = %" PRId64 ANSI_COLOR_RESET "\n",
                icell,
                (ix * nmesh_y * nmesh_z + iy * nmesh_z + (int64_t) iz));
        ...
        for (int iix = -xbin_refine_factor; iix <= xbin_refine_factor; iix++) {
            ...
            for (int iiy = -ybin_refine_factor; iiy <= ybin_refine_factor; iiy++) {
                ...
                for (int64_t iiz = -zbin_refine_factor; iiz <= zbin_refine_factor;
                     iiz++) {
                    const int iiiz = iz + iiz;
                    if (iiiz < 0 || iiiz >= nmesh_z) continue;

                    const int64_t icell2 =
                       iiiz + (int64_t) nmesh_z * iiiy + nmesh_z * nmesh_y * iiix;

                    // Since we are creating a giant array with all possible cell-pairs,
                    // we need to account for cases where an auto-correlation is
                    // occurring within the same cell. To do so, means 'same_cell',
                    // 'min_dx/dy/dz', and 'closest_x1/y1/z1' must all be set here.
                    // Also, if the second cell has no particles, then just skip it
                    if ((autocorr == 1 && icell2 > icell)
                        || lattice2[icell2].nelements == 0) {
                        continue;
                    }
                    ...
                    if (enable_min_sep_opt && max_3D_sep > 0) {
                        ...
                        const DOUBLE x_low = first->xbounds[0],
                                     x_hi  = first->xbounds[1];
                        const DOUBLE y_low = first->ybounds[0],
                                     y_hi  = first->ybounds[1];
                        const DOUBLE z_low = first->zbounds[0],
                                     z_hi  = first->zbounds[1];

                        closest_x1 = iix < 0 ? x_low : (iix > 0 ? x_hi : ZERO);
                        closest_y1 = iiy < 0 ? y_low : (iiy > 0 ? y_hi : ZERO);
                        closest_z1 = iiz < 0 ? z_low : (iiz > 0 ? z_hi : ZERO);

                        // chose lower bound if secondary cell is to the left (smaller
                        // x), else upper
                        const DOUBLE first_x =
                           iix < 0 ? x_low :
                                     x_hi;  // second condition also contains iix==0
                        // chose upper bound if primary cell is to the right (larger x),
                        // else lower
                        const DOUBLE second_x =
                           iix < 0 ?
                              second->xbounds[1] :
                              second
                                 ->xbounds[0];  // second condition also contains iix==0
                        min_dx = iix != 0 ? (first_x - second_x) :
                                            ZERO;  // ensure min_dx == 0 for iix ==0

                        // repeat for min_dy
                        const DOUBLE first_y =
                           iiy < 0 ? y_low :
                                     y_hi;  // second condition also contains iiy==0
                        const DOUBLE second_y =
                           iiy < 0 ?
                              second->ybounds[1] :
                              second
                                 ->ybounds[0];  // second condition also contains iiy==0
                        min_dy = iiy != 0 ? (first_y - second_y) :
                                            ZERO;  // ensure min_dy == 0 for iiy ==0

                        // repeat for min_dz
                        const DOUBLE first_z =
                           iiz < 0 ? z_low :
                                     z_hi;  // second condition also contains iiz==0
                        const DOUBLE second_z =
                           iiz < 0 ?
                              second->zbounds[1] :
                              second
                                 ->zbounds[0];  // second condition also contains iiz==0
                        min_dz = iiz != 0 ? (first_z - second_z) :
                                            ZERO;  // ensure min_dz == 0 for iiz ==0

                        const DOUBLE sqr_min_sep_cells =
                           min_dx * min_dx + min_dy * min_dy + min_dz * min_dz;
                        if (sqr_min_sep_cells >= max_3D_sep * max_3D_sep) { continue; }
                    ...
                    XRETURN(
                       num_cell_pairs < max_num_cell_pairs,
                       NULL,
                       "Error: Assigning this existing cell-pair would require accessing invalid memory.\n"
                       "Expected that the total number of cell pairs can be at most %" PRId64
                       " but "
...
cellarray_mocks *gridlink_mocks_theta_dec(const int64_t NPART,
    ...
    /* Input validation */
    XRETURN(thetamax > 0.0,
            NULL,
            "Minimum angular separation = %" REAL_FORMAT " must be positive\n",
            thetamax);
    XRETURN(
       dec_diff > 0.0,
       NULL,
       "All of the points can not be at the same declination. Declination difference = %" REAL_FORMAT
       " must be non-zero\n",
       dec_diff);
    ...
    XRETURN(totncells != NULL,
            NULL,
            "Pointer to return the total number of cells must be a valid address\n");
    XRETURN(options != NULL,
            NULL,
            "Structure containing code options must be a valid address\n");

    /* Protect against accidental changes to compile-time (macro) constant */
    XRETURN(MEMORY_INCREASE_FAC >= 1.0,
            NULL,
            "Memory increase factor = %lf must be >=1 \n",
            MEMORY_INCREASE_FAC);
    ...
    int ngrid_dec =
       this_ngrid_dec_int > max_dec_size ? max_dec_size : this_ngrid_dec_int;
    XRETURN(
       ngrid_dec >= dec_refine_factor,
       NULL,
       "Number of grid cells = %d must be at least the declination refinement factor = %d\n",
       ngrid_dec,
       dec_refine_factor);
    ...
    *totncells = ngrid_dec;
    ...
    cellarray_mocks *lattice =
       (cellarray_mocks *) my_calloc(sizeof(*lattice), ngrid_dec);
    ...
    if (options->copy_particles) {
        ...
        original_indices = (int64_t *) my_malloc(sizeof(*original_indices), NPART);
        totnbytes +=
           sizeof(*all_cell_indices) * NPART + sizeof(*original_indices) * NPART;
    }
    if (lattice == NULL || (options->copy_particles == 0 && all_cell_indices == NULL)
        || (options->copy_particles == 0 && original_indices == NULL)
        || (options->copy_particles && nallocated == NULL)) {
    ...
    for (int icell = 0; icell < ngrid_dec; icell++) {
        ...
        if (options->copy_particles) {
            ...
            lattice[icell].original_index =
               my_malloc(sizeof(*(lattice[icell].original_index)), expected_n);
            ...
            nallocated[icell] = expected_n;
            totnbytes +=
               3 * memsize * expected_n
               + sizeof(int64_t) * expected_n;  // assumes original_index is 64 bit
        }
    }
    ...
    for (int64_t i = 0; i < NPART; i++) {
        ...
        XRETURN(idec >= 0 && idec < ngrid_dec,
                NULL,
                "idec (dec bin index) = %d must be within [0, %d)",
                idec,
                ngrid_dec);
        ...
        if (options->copy_particles == 0) {
            ...
            if (lattice[icell].nelements == nallocated[icell]) {
                expected_n = nallocated[icell] * MEMORY_INCREASE_FAC;
                while (expected_n <= lattice[icell].nelements) { expected_n++; }
            ...
            XRETURN(lattice[icell].nelements < nallocated[icell],
                    NULL,
                    ANSI_COLOR_RED
                    "BUG: lattice[%d].nelements = %" PRId64
                    " must be less than allocated memory = %" PRId64 ANSI_COLOR_RESET
                    "\n",
                    idec,
                    lattice[icell].nelements,
        ...
        lattice[icell].dec_bounds[0] = DEC[i] < lattice[icell].dec_bounds[0] ?
                                          DEC[i] :
                                          lattice[icell].dec_bounds[0];
        lattice[icell].dec_bounds[1] = DEC[i] > lattice[icell].dec_bounds[1] ?
                                          DEC[i] :
                                          lattice[icell].dec_bounds[1];

        lattice[icell].ra_bounds[0] =
           RA[i] < lattice[icell].ra_bounds[0] ? RA[i] : lattice[icell].ra_bounds[0];
        lattice[icell].ra_bounds[1] =
           RA[i] > lattice[icell].ra_bounds[1] ? RA[i] : lattice[icell].ra_bounds[1];
...
#undef MULTIPLE_ARRAY_EXCHANGER
    ...
    if (options->verbose) {
        ...
        gettimeofday(&t1, NULL);
        fprintf(stderr,
                "%s> expected_n = %" PRId64 " ngrid (declination) = %d np=%" PRId64
                " Memory required = %0.2lf MB. Time taken = %7.3lf sec \n",
                __FUNCTION__,
                expected_n,
                ngrid_dec,
                NPART,
                totnbytes / 1024.0 / 1024.,
                ADD_DIFF_TIME(t0, t1));
    }
...
struct cell_pair *generate_cell_pairs_mocks_theta_dec(cellarray_mocks *lattice1,
    ...
    /* This ngb is a trivial function. Loop over +- idec from every cell. And that's a
     * neighbour cell */
    for (int64_t icell = 0; icell < totncells; icell++) {
        cellarray_mocks *first = &(lattice1[icell]);
        if (first->nelements == 0) continue;
        ...
        for (int idec = -dec_refine_factor; idec <= dec_refine_factor; idec++) {
            ...
            // to account for cases where an auto-correlation is occurring within the
            // same cell. To do so, means 'same_cell', 'min_dx/dy/dz', and
            // 'closest_x1/y1/z1' must all be set here. Also, if the second cell has no
            // particles, then just skip it
            ...
            if (enable_min_sep_opt && idec != 0) {
                const DOUBLE first_z_low  = first->zbounds[0],
                             first_z_hi   = first->zbounds[1];
                const DOUBLE second_z_low = second->zbounds[0],
                             second_z_hi  = second->zbounds[1];
                ...
                const DOUBLE sqr_min_sep_cells =
                   min_dx * min_dx + min_dy * min_dy + min_dz * min_dz;
                if (sqr_min_sep_cells >= sqr_max_chord_sep) { continue; }
            }
...
cellarray_mocks *gridlink_mocks_theta_ra_dec(
   const int64_t NPART,
   DOUBLE *RA,
   DOUBLE *DEC,
   DOUBLE *X,
   ...
   const config_options *options) {
    ...
    XRETURN(options != NULL,
            ...
            "Structure containing code options must be a valid address\n");
    XRETURN(
       ngrid_declination != NULL,
       NULL,
       "Address to return the number of DEC cells must be valid (ngrid_declination is NULL)\n");
    ...
    XRETURN(ra_diff > 0.0,
            NULL,
            "All of the points can not be at the same RA. RA difference = %" REAL_FORMAT
            " must be non-zero\n",
            ra_diff);
    ...
    *ngrid_phi = my_malloc(sizeof(**ngrid_phi), ngrid_dec);
    ...
    // Just make sure that the assign_ngb does not keep adding the same cell over and
    // over
    int max_nmesh_phi =
       1;  // at least one cell (at the poles, there might be only one cell).
    const DOUBLE max_phi_cell =
       ra_diff;  // must have at least one cell -> max ra binsize := ra_diff/1.0;
    for (int idec = 0; idec < ngrid_dec; idec++) {
        ...
        // Use the Haversine approx.
        // (https://en.wikipedia.org/wiki/Great-circle_distance)
        //  \delta\sigma = 2 asin ( sqrt ( sin^2( 1/2 (phi1 - phi2)) + cos(phi1) *
        //  cos(phi2) * sin^2( 1/2 max-ra-diff)) ) Since phi1 == phi2 = this_min_dec;
        //  cos(this_min_dec) := cos_min_dec
        ...
        if ((90.0 - ABS(this_min_dec))
            > 1.0) {  // make sure min_dec is not close to the pole (within 1 degree)->
                      // divide by zero happens the cosine term
            // Haversine formula but numerical precision issues might cause
            // sin_half_thetamax/cos_min_dec > 1.0 or sin_half_thetamax/cos_min_dec <
            // -1.0 at which point arcsin() will barf. Therefore, a two-step process to
            // limit the possible values. Sorry about the pythonic naming convention +
            // the names themselves - MS 3rd April, 2017
            const DOUBLE _tmp =
               sin_half_thetamax
               / cos_min_dec;  // sin(1/2 thetamax) > 0, cos(min_dec) >= 0 ( -90 <=
                               // declination <= 90, thetamax > 0)
            ...
            // previous phi_cell calculation is split up into two steps - MS 3rd April,
            // 2017
            if (phi_cell <= ZERO) { phi_cell = max_phi_cell; }

            // NAN's might trigger this condition ?
            if (!(phi_cell > ZERO)) {
                fprintf(stderr,
                        "Error: Encountered invalid binsize for RA bins = %" REAL_FORMAT
                        " for declination bin = %d\n",
                        phi_cell,
                        idec);
                fprintf(stderr,
                        "min(declination in bin) = %" REAL_FORMAT " (min_dec)\n",
                        this_min_dec);
                fprintf(stderr,
                        "costhetamax = %" REAL_FORMAT
                        " sin(thetamax/2.0) = %" REAL_FORMAT
                        " cos(min_dec) = %" REAL_FORMAT " \n",
                        costhetamax,
                        sin_half_thetamax,
                        cos_min_dec);
            }
        }
        XRETURN(phi_cell > ZERO,
                NULL,
                "Please remove LINK_IN_RA from compile options "
                "(or from the code config options)\n");

        phi_cell = phi_cell > max_phi_cell ? max_phi_cell : phi_cell;
        const DOUBLE this_nmesh_ra =
           (ra_diff / phi_cell < 1) ?
              1 :
              ra_diff / phi_cell;  // should be >= 1 since phi_cell is <= ra_diff
        const int this_nmesh_ra_int = ((int) this_nmesh_ra) * ra_refine_factor;
        int nmesh_ra =
           this_nmesh_ra_int > max_ra_size ? max_ra_size : this_nmesh_ra_int;

        // at least one ra bin
        if (nmesh_ra < 1) { nmesh_ra = 1; }
    ...
    *max_nmesh_ra = max_nmesh_phi;
    expected_n =
       (int64_t) ((NPART / (DOUBLE) (ngrid_dec * max_nmesh_phi)) * MEMORY_INCREASE_FAC);
    expected_n = expected_n < 2 ? 2 : expected_n;
    ...
    for (int64_t i = 0; i < NPART; i++) {
        ...
        if (options->copy_particles == 0) {
            ...
            if (lattice[icell].nelements == nallocated[icell]) {
                ...
                totnbytes += 3 * (expected_n - nallocated[icell]) * memsize
                             + (expected_n - nallocated[icell]) * sizeof(int64_t);
                nallocated[icell] = expected_n;
            }
            XRETURN(lattice[icell].nelements < nallocated[icell],
                    NULL,
    ...
    {                                                                                \
        ...
        for (int w = 0; w < first->weights.num_weights; w++) {                       \
            ...
            if (num_sorted < first->nelements) {
                ...
                //(since z := sin(dec) is a monotonic mapping in -90 <= dec <= 90, the
                // domain for dec)
                if (options->use_heap_sort
                    || num_sorted
                          > FRACTION_SORTED_REQD_TO_HEAP_SORT * first->nelements) {
...
#undef MULTIPLE_ARRAY_EXCHANGER
    ...
    if (options->verbose) {
        ...
        fprintf(
           stderr,
           "%s> Max. points in cell <= %" PRId64
           " ngrid (declination) = %d max ngrid (ra) = %d. Number of points = %" PRId64
           " Memory required = %0.2g MB."
...
struct cell_pair *generate_cell_pairs_mocks_theta_ra_dec(cellarray_mocks *lattice1,
    ...
    XRETURN(
       ...
       NULL,
       "Total number of cells = %" PRId64
       " can be at most the product of max RA cells = %d and the number of DEC cells = %d\n",
       totncells,
       max_ngrid_ra,
       ngrid_dec);

    /* Since lattice1 is a single dimension array containing dec + ra bins, where the
       number of ra bins per dec bin varies, I need to have the offset (array index,
       **NOT* pointer offset) for the starting ra cell for each dec bin. Note: lattice2
       *must* be of the exact same shape and there the same offsets apply to both
       lattices
    */
    int64_t *ra_offset_for_dec = my_malloc(sizeof(*ra_offset_for_dec), ngrid_dec);
    if (ra_offset_for_dec == NULL) { return NULL; }
    ...
    XRETURN(
       ...
       NULL,
       "Total number of cells = %" PRId64
       " must be exactly equal to the sum of number of RA cells over all declinations = %" PRId64
       "\n",
       totncells,
       offset);
    ...
    for (int idec = 0; idec < ngrid_dec; idec++) {
        for (int ira = 0; ira < ngrid_ra[idec]; ira++) {
            ...
            for (int dec_refine = -dec_refine_factor; dec_refine <= dec_refine_factor;
                 dec_refine++) {
                const int this_dec = idec + dec_refine;
                if (this_dec < 0 || this_dec >= ngrid_dec) continue;

                /* Figure out what is the min and max RA cell that any particle in
                   "first" cell could be in if the "first" cell particle had a
                   declination for this_dec (rather than idec) */
                // include one additional cell -> will get pruned if the separation is
                // too large *and* never be duplicated brute force approach would be to
                // simply loop from 0 to ngrid_ra[this_dec] and prune away.
                const int min_ra_this_dec =
                   (int) (ngrid_ra[this_dec] * (first->ra_bounds[0] - ra_min)
                          * inv_ra_diff)
                   - 1;
                const int max_ra_this_dec =
                   (int) (ngrid_ra[this_dec] * (first->ra_bounds[1] - ra_min)
                          * inv_ra_diff)
                   + 1;

                for (int iira = min_ra_this_dec - ra_refine_factor;
                     iira <= max_ra_this_dec + ra_refine_factor;
                     iira++) {
                    int this_ra = iira + ngrid_ra[this_dec];
                    while (this_ra < 0) { this_ra += ngrid_ra[this_dec]; }
                    this_ra = this_ra % ngrid_ra[this_dec];

                    XRETURN(
                       this_ra >= 0 && this_ra < ngrid_ra[this_dec],
                       NULL,
                    ...
                    XRETURN(icell2 < totncells,
                            NULL,
                            "index for ngb cell = %" PRId64
                            " should be less total number of cells = %" PRId64 "\n",
                            icell2,
                            totncells);

                    cellarray_mocks *second = &(lattice2[icell2]);
                    // For cases where we are not double-counting (i.e., auto-corrs),
                    // the same-cell must always be evaluated. In all other cases,
                    // (i.e., where double-counting is occurring) is used, include that
                    // in the ngb_cells! The interface is a lot cleaner in the
                    // double-counting kernels in that case! The second condition
                    // essentially halves the number of cell-pairs in auto-corr
                    // calculations
                    ...
                    CHECK_AND_CONTINUE_FOR_DUPLICATE_NGB_CELLS(icell,
                                                               icell2,
                                                               xwrap,
                                                               ywrap,
                                                               zwrap,
                    ...
                    if (enable_min_sep_opt) {
                        DOUBLE closest_pos0;
                        min_dx = find_closest_pos(
                           first->xbounds, second->xbounds, &closest_pos0);
                        closest_x1 = closest_pos0;

                        min_dy = find_closest_pos(
                           first->ybounds, second->ybounds, &closest_pos0);
                        closest_y1 = closest_pos0;

                        min_dz = find_closest_pos(
                           first->zbounds, second->zbounds, &closest_pos0);
                    ...
                    num_cell_pairs++;
                    num_ngb_this_cell++;
                }  // loop over possible range of RA values in this dec bin for the
                   // original RA bin in first
